...
for (filename in datasets_filenames) {
  ...
  for (j in datasets) {
    ...
    if(filename =="10_7717_peerj_5665_dataYM2018_neuroblastoma.csv"){
      cl51_db<-dbscan(j, eps = 4.9, minPts = 13)
      cl52_db<-dbscan(j, eps = 4.9, minPts = 13)
      ...
      cat("Risultato Gap statistic con dbscan su primo dataset: ",gap_statInd1_db$gap, "\n", sep = "")
     
      
    }
    if(filename =="journal.pone.0148699_S1_Text_Sepsis_SIRS_EDITED.csv"){
      cl52_db<-dbscan(j, eps = 78, minPts = ncol(j)+1)
      cl62_db<-dbscan(j, eps = 78, minPts = ncol(j)+1)
      ...
      cat("Risultato Gap statistic con dbscan su secondo dataset: ", gap_statInd2_db$gap, "\n", sep = "")
      
    }
    if(filename == "journal.pone.0158570_S2File_depression_heart_failure.csv"){
      cl53_db<-dbscan(j, eps = 220, minPts = ncol(j)+1)
      cl63_db<-dbscan(j, eps = 220, minPts = ncol(j)+1)
      ...
      cat("Risultato Gap statistic con dbscan su terzo dataset: ", gap_statInd3_db$gap, "\n", sep = "")
    }
    if(filename =="journal.pone.0175818_S1Dataset_Spain_cardiac_arrest_EDITED..csv" ){
      cl54_db<-dbscan(j, eps=7.5, minPts = ncol(j)+1)
      cl64_db<-dbscan(j, eps = 7.5, minPts = ncol(j)+1)
      ...
      cat("Risultato Gap statistic con dbscan su quarto dataset: ", gap_statInd4_db$gap, "\n", sep = "")
      
    }
    if(filename == "Takashi2019_diabetes_type1_dataset_preprocessed.csv"){
      cl55_db<-dbscan(j, eps=26.5, minPts = ncol(j)+1)
      cl65_db<-dbscan(j, eps = 26.5, minPts = ncol(j)+1)
      ...
      cat("Risultato Gap statistic con dbscan su quinto dataset: ", gap_statInd5_db$gap, "\n", sep = "")
    ...
    cat("Risultato Gap statistic con dbscan utilizzando un diverso eps: ", gap_statInd12_db$gap, "\n", sep = "")
    dbs13<-dbscan(j, eps=eps2, minPts = eps2+1)
    dbs23<- dbscan(j, eps=eps2, minPts = eps2+1)
    clustertot13_db<-cbind(dbs13$cluster, dbs23$cluster)
    gap_statInd13_db<-index.Gap(j, clustertot13_db, reference.distribution = "unif", B=bootstrap_clus, centrotypes = "centroids")
    cat("Risultato Gap statistic con dbscan utilizzando l'eps precedente ma un diverso numero minimo di punti: ", gap_statInd13_db$gap, "\n", sep = "")
    cat("", sep = "\n\n")
    break
