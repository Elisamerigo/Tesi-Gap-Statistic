...
gap_stat1<-index.Gap(matrice,clustertot,reference.distribution = "unif",B=10, method = "k-means",centrotypes = "centroids")
...
for (f in d) {
  for (g in 1:ncol(matrice)) {
      ...
      if (g==ncol(matrice)){
        ...
        gap_stat2<-index.Gap(matrice,clustertot1,reference.distribution = "unif",B=10,method = "k-means", centrotypes = "centroids")
...
ggplot(my_data2, aes(x=n_righe2, y=gapC1))+geom_point(shape=20, color="blue")+geom_smooth(method = lm, color="red")+labs(title = "Andamento Gap statistic con clusGap primo", x="numero righe manipolate", y="Gap")
...
ggplot(my_data2_senza_primopunto, aes(x=n_righe, y=gapC1_senza_primopunto))+geom_point(shape=20, color="blue")+geom_smooth(method = lm, color="red")+labs(title = "Andamento Gap statistic con clusGap primo senza primo punto", x="numero righe manipolate", y="Gap")
