...
set.seed(27)
#matrice caso 1
matrice_caso1<- matrix(data = 2, nrow = 6, ncol = 6)
...
#Con clusGap
gapclus_caso1<- clusGap(matrice_caso1, FUN = kmeans, nstart=25, K.max = 2, B=10)
print(gapclus_caso1)
...
gapInd_caso1<-index.Gap(matrice_caso1, clustertot_caso1, reference.distribution = "unif", B=10, method = "k-means", centrotypes = "centroids")
...
n_min<-8
n_max<-9
matrice_caso2<- matrix(data=0, nrow = 6, ncol = 6)
...
#Con clusGap
gapclus_caso2<-clusGap(matrice_caso2, FUNcluster = kmeans, nstart=25, K.max=2, B=10)
print(gapclus_caso2)
...
gapInd_caso2<-index.Gap(matrice_caso2, clustertot2_caso2, reference.distribution = "unif", B=10, method = "k-means", centrotypes = "centroids")
